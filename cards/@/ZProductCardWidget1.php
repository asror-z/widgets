...
class ZProductCardWidget1 extends ZWidget

{
    ...
    public $_config = [
        ...
        'price' => '$500',
        'price_old' =>'$1000',
        'card_event' => ZProductCardWidget1::card_name['none'],
        'class' => 'zetColor',
...
<div class="card">
  ...
  <div class="view overlay">
    ...
    <a>
      <div class="mask rgba-white-slight">AasASasAS</div>
    </a>
  </div>
    <div class="hover_box">
...
JS,
    ...
    public function codes()
    {
        $color = '';
        $display = '';
        $colorNew = ZProductCardWidget1::card_name['new'];
        $colorSale = ZProductCardWidget1::card_name['sale'];
        $colorNone = ZProductCardWidget1::card_name['none'];

        if($colorNew === true) {
            $color = ZProductCardWidget1::event_color['blue'];
        }else {

        }

        if($colorSale === true) {
            $color = ZProductCardWidget1::event_color['red'];
        }else {
        }

        if($colorNone === true) {
            $color = '';
            $display = ZProductCardWidget1::event_color['none'];
        } else {
