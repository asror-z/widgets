 ...
 *
 */
class ZChartFormWidgetJaxongir extends ZWidget
{
    ...
    private $aChart_Series_Line_Stack = [
        'areaStyle' => [
            'normal' => []
        ],
    ...
    public function codes()
    {
        ...
        foreach ($this->data as $column){
            if(!empty($column->{$this->_config['value']}))
            $keys[] = $column->{$this->_config['name']};
        ...
        if(!empty($item->{$this->_config['value']})){
            if($this->_config['color'])
                $color[]=$item->{$this->_config['colorname']};
            $data[1][] = $item->{$this->_config['value']};
        ...
        if($this->_config['color'])
            $this->aChart_Series_Pie['color']  =$color;
        ...
        if (!empty($this->data)) {
            {
                ...
                $this->aChart_Series_Pie = ArrayHelper::merge(
                    $this->aChart_Series_ALL,
                    $this->aChart_Series_Pie
                );

                $this->aChart_Series_Line = ArrayHelper::merge(
                    $this->aChart_Series_ALL,
                    $this->aChart_Series_Line
                );

                $this->aChart_Series_Bar = ArrayHelper::merge(
                    $this->aChart_Series_ALL,
                    $this->aChart_Series_Bar
                );

                $this->aChart_Series_Line_Stack = ArrayHelper::merge(
                    $this->aChart_Series_Line,
                    $this->aChart_Series_Line_Stack
                );

                $this->aChart_XAxis = ArrayHelper::merge(
                    $this->aChart_XAxis,
                    $this->chartAxisCore
                );

                $this->aChart_YAxis = ArrayHelper::merge(
                    $this->aChart_YAxis,
                    $this->chartAxisCore
                );
                ...
                switch ($this->_config['type']) {
                    case self::type['line']:
                        $aSeries = $this->aChart_Series_Line;
                        break;

                    case self::type['lineStack']:
                        $aSeries = $this->aChart_Series_Line_Stack;
                        break;

                    case self::type['bar']:
                        $aSeries = $this->aChart_Series_Bar;
                        break;

                    case self::type['pie']:
                        $aSeries = $this->aChart_Series_Pie;
                        break;
                }
                ...
                if ($this->_config['type'] !== self::type['pie']) {
                    ...
                    $this->option['grid'] = $this->aChart_Grid;
                    $this->option['xAxis'] = $this->aChart_XAxis;
                    $this->option['yAxis'] = $this->aChart_YAxis;
                    ...
                    $this->aChart_DataZoom[0] = ArrayHelper::merge(
                        $this->aChart_DataZoom[0],
                        $aDataZoom
                    );

                    $this->aChart_DataZoom[1] = ArrayHelper::merge(
                        $this->aChart_DataZoom[1],
                        $aDataZoom
                    );

                    $this->option['dataZoom'] = $this->aChart_DataZoom;
                    //vdd($this->option);

                }
                else {
                    $this->aChart_Tooltip['trigger'] = self::trigger['item'];
                    $this->aChart_Tooltip['position'] = null;
                }

                $this->option['title'] = $this->aChart_Title;
                $this->option['tooltip'] = $this->aChart_Tooltip;
                $this->option['axisPointer'] = $this->aChart_AxisPointer;
                $this->option['toolbox'] = $this->aChart_Toolbox;
                $this->option['textStyle'] = $this->aChart_Textstyle;
                $this->option['legend'] = $this->aChart_Legend;
