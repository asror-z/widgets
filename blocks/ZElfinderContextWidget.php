 ...
 */
class ZElfinderContextWidget extends ZWidget
{
    ...
    public $_config = [
        ...
        'type' => [],
        'handler' => "",
        // 'handler' => "/core/tester/asror/main.aspx?path=render/{fileName}"
        // 'handler' => "/render/{fileName}"
    ];
...
HTML,
        ...
        elFinder.prototype._options.commands.push('test');
        elFinder.prototype._options.contextmenu.files.push('test');
        elFinder.prototype.i18.en.messages['cmdtest'] = 'Open in new tab';
        ...
        elFinder.prototype.commands.test = function() {
          this.exec = function(hashes , fm) {
            //implement what the custom command should do here
               /*var myCommands = Object.keys( elFinder.prototype._options.getFileCallback);*/
                  /*console.log(elFinder.prototype._options.ui.path);*/
                //console.log(elFinder.prototype._options.bootCallback);
                    let el = $('.ui-selected').find('.elfinder-cwd-filename').attr('title');    
                    let title = fm._currentNode[0].ownerDocument.title;        
                    let path = title.includes(':')? title.split(':')[0]:title;
                    path = path.substr( path.indexOf('\\\'));
                    let url = path+'/'+el.split('.')[0]+'.aspx';
                    let hostname = 'http://www.'+window.location.hostname; 
                    url = hostname +'/'+url.replace('\\\','/');                     
        ...
        elFinder.prototype.i18.ru.messages.cmdcopy = 'Edit in Grape';
        elFinder.prototype._options.commands.push('grape');
        elFinder.prototype._options.contextmenu.files.push('grape');
        elFinder.prototype.i18.en.messages.grape = 'Edit in Grape';
        
        elFinder.prototype.commands.grape = function() {
          this.exec = function(hashes,fm) {
            //implement what the custom command should do here
                let el = $('.ui-selected').find('.elfinder-cwd-filename').attr('title');    
                let hostname = 'http://www.'+window.location.hostname; 
                let grapes = '/core/widget/sampleWidgetNormS.aspx?dir='
                let title = fm._currentNode[0].ownerDocument.title;
                title = title.substr( 0,path.indexOf(':'))    
            ...
            $('#{id}').elfinder(
                  ...
                  
                {
                    ...
                    function str_replace() {
                           // var fileNameFull = root[0] + '/' + atob(encPath);
                           ...
                           //console.log(fullUrl,fileNameFull)
                           //return fullUrl;
                           return fileNameFull;
                    }
                    ...
                    if (allowed.includes(fileMime)){
                             $("#" + id).dblclick(function(event){
                              if (!$("#"+id).hasClass('directory')){
                                  ...
                                  window.open(str_replace(), '_blank');
                              }
