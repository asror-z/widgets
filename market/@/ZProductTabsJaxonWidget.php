...
class ZProductTabsJaxonWidget extends ZWidget
{
    ...
    public $_config = [
        ...
        'pager' => ZProductTabsJaxonWidget::type['scroll'],
        'addButton' => false,
        'cols' => 3,
        'sort' => ['price'],
        'key'=> [SORT_DESC],
...
CSS,
    ...
    {$sort=$this->httpGet('sort');
        ...
        if (isset($sort)){
            $names=explode(" ", $sort);
            foreach ($names as $key => $value) if($names[$key]!="") $array[]=$names[$key]/*array_splice($names, $key, 1)*/;
            $names=$array;
