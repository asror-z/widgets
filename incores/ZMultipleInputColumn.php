...
use zetsoft\system\helpers\ZValuePreparer;
 ...
 */
class ZMultipleInputColumn extends BaseObject
{
...
//vd($type, $name, $value, $options);
        ...
        // vd($this->context->model->id);

        // vd($this->name);
        // vd($type, $name, $value, $options);

        $parentModel = $this->context->model;
        $parentAttr = $this->context->attribute;
        ...
        $index = Az::$app->utility->pregs->multiIndex($options['id']);
        //vd($index);

        if ($index !== null) {

            $value = ZArrayHelper::getValue($values, $index);
            //  vd($index,$values);
        ...
        $column = $model->columns[$options['attribute']];
        ...
        if ($type instanceof ZWidget)
            $options = ZArrayHelper::merge($options, $core, [
                'config' => [
                    'isMulti' => true,
                    'parentWidget' => ZWidget::parents['multiwidget'],
    ...
    public function prepareValue($contextParams = [])
    {
        $data = $this->getModel();
        ...
        if ($this->value instanceof \Closure) {
            ...
            // fix 0
            $valuePreparer = new ZValuePreparer($this->name, $this->defaultValue);
    ...
    public function getElementName($index, $withPrefix = true)
    {
        ...
        $hasOne = $this->isRendererHasOneColumn();
        ...
        $name = $prefix . $elementName . (empty($this->nameSuffix) ? '' : ('_' . $this->nameSuffix));
    ...
    public function renderInput($name, $options, $contextParams = [])
    {
        ...
        $options['id'] = $contextParams['id'];
        $options['name'] = $contextParams['name'];
