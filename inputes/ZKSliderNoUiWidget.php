...
class ZKSliderNoUiWidget extends ZWidget
{
    public $config = [];
    public $_config = [
        'start' => [50,70],
...
CSS,
    ...
    public function codes()
    {
        if(isset($this->config['format']))
            $this->config['format'] = array_merge($this->_config['format'], $this->config['format']);
        else $this->config['format'] = $this->_config['format'];
        ...
        $this->js = strtr($this->_layout['js'], [
            ...
            "'*': '*'" => $widget_configs,
            '{id}' => $this->id,
            "{start}" => ($this->modelCheck() && isset($this->value)) ?json_encode($this->value): json_encode($this->config['start']),
            "{format}" => json_encode($this->config['format']),
            "{format_save}" => '{decimals:'.$this->config['format']['decimals'].'}',
        ]);
    ...
    private function WidgetConfigGenerate()
    {
        ...
        $count = ($this->modelCheck() && isset($this->value)) ?count($this->value): count($this->config['start']);
