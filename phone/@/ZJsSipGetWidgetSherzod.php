...
class ZJsSipGetWidgetSherzod extends ZWidget
{
...
HTML,
    ...
    var socket = new JsSIP.WebSocketInterface('{socket}');
    ...
    callButton.addEventListener('click', function () {
        phone.call('sip:' + txtPhoneNumber.value + '{uri}', options)
        add_stream();
    });
...
JS
    ...
    {
        foreach ($this->data as $JSSIPItem){
        $this->htm = strtr($this ->_layout['main'],[
            '{keypad}' => $JSSIPItem->keypad ? '' : "d-none"
        ]);

        $this->js = strtr($this->_layout['js'], [
            '{uri}' => $JSSIPItem->impu,
            '{impi}' => $JSSIPItem->impi,
            '{passWord}' => $JSSIPItem->password,
            '{socket}' => $JSSIPItem->websocket_proxy_url,
