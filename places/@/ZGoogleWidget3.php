 ...
 * Created By: Daho
 */
class ZGoogleWidget3 extends ZWidget
{
    ...
    public $_config = [
        ...
        'mapType' => self::mapType['SATELLITE'],
        'scrollwheel' => true,
        'zoomControl' => true,
        'zoomControlPosition' => self::zoomControlPosition['TOP_LEFT'],
        'originLat'=> 37.77,   // isShowMode true
        'originLng'=> -122.447, //isShowMode true
        'destinationLat'=> 38.447, //isShowMode true
        'destinationLng'=> -122.447, // isShowMode true
        'isBermuda'=> false,
        'isShowMode' => false,
        'isShowHeatMap' =>false,
    ...
    {
        // $this->fileJs('https://maps.googleapis.com/maps/api/js?key=AIzaSyBkxS5l87lclaC6MIWSGejdCXL13wSShRo&libraries=drawing,visualization,places&callback=initMap');
    }
    ...
    public function codes()
    {
        $this->layout = [
            'main' => <<<HTML
            <button id="btn-location">{myLocation}</button>
    ...
    <script>
     ...
     var isShowMode = {isShowMode};
     var isShowHeatMap = {isShowHeatMap}
     var isDrawingManager = {isDrawingManager}
     var isGetLocation = {isGetLocation}
     var isGetJson = {isGetJson}
     var isSearchBox = {isSearchBox}
        ...
        function initMap() {
            ...
            map = new google.maps.Map(document.getElementById("map"), {
                center: {lat: 41.3069288, lng: 69.2760709},//, 
                zoom: {zoom},
             ...
             */
             if (isSearchBox===true){
                     // Create the safe box and link it to the UI element.
        ...
        directionsService.route({
            origin: {lat: {originLat}, lng: {originLng}},  // Haight 37.77  -122.447.
            destination: {lat: {destinationLat}, lng: {destinationLng}},  // Ocean Beach.
...
HTML,
        ...
        $this->htm = strtr($this->layout['main'], [
            ...
            '{toggle}' => Az::l("Включить тепловую карту"),
            '{radius}' => Az::l("Изменить градиент"),
            '{opacity}' => Az::l("Изменить прозрачность"),
            '{gradient}' => Az::l("Изменить градиент"),
            '{search}' => Az::l("Поиск"),
            '{myLocation}' => Az::l("Моя локация"),
            '{driving}' => Az::l("На машине"),
            '{walking}' => Az::l("Пешком"),
            '{bicycling}' => Az::l("На велосипеде"),
            '{transit}' => Az::l("Транзит"),
            '{modeTraveling}' => Az::l("Режим путешествия"),
            '{disableDoubleClickZoom}' => $this->jscode($this->_config['disableDoubleClickZoom']),
            '{fullScreenControlPosition}' => $this->jscode($this->_config['fullScreenControlPosition']),
            '{MapTypeControlStyle}' => $this->jscode($this->_config['MapTypeControlStyle']),
            '{mapTypeControlPosition}' => $this->jscode($this->_config['mapTypeControlPosition']),
            '{mapType}' => $this->jscode($this->_config['mapType']),
            '{scrollwheel}' => $this->jscode($this->_config['scrollwheel']),
            '{zoomControl}' => $this->jscode($this->_config['zoomControl']),
            '{zoomControlPosition}' => $this->jscode($this->_config['zoomControlPosition']),
            '{originLat}' => $this->jscode($this->_config['originLat']),
            '{originLng}' => $this->jscode($this->_config['originLng']),
            '{destinationLat}' => $this->jscode($this->_config['destinationLat']),
            '{destinationLng}' => $this->jscode($this->_config['destinationLng']),
			'{isBermuda}'  => $this->jscode($this->_config['isBermuda']),
			'{isShowMode}'=>$this->jscode($this->_config['isShowMode']),
			'{isShowHeatMap}'=>$this->jscode($this->_config['isShowHeatMap']),
			'{isDrawingManager}'=>$this->jscode($this->_config['isDrawingManager']),
			'{isGetLocation}'=>$this->jscode($this->_config['isGetLocation']),
			'{isGetJson}'=>$this->jscode($this->_config['isGetJson']),
			'{isSearchBox}'=>$this->jscode($this->_config['isSearchBox']),
			'{isKmlLayer}'=>$this->jscode($this->_config['isKmlLayer']),
