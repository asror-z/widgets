...
class ZGoogleZoirNavigationWidgetS extends ZWidget
{
    #region Congif
    /**
     * Configuration
...
&photoreference=CnRtAAAATLZNl354RwP_9UKbQ_5Psy40texXePv4oAlgP4qNEkdIrkyse7rPXYGd9D_Uj1rVsQdWT4oRz4QrYAJNpFX7rzqqMlZw2h2E2y5IKMUZ7ouD_SlcHxYq1yL4KbKUv3qtWgTK0A6QbGh87GB3sscrHRIQiG2RrmU_jF4tENr9wGS_YxoUSSDrYjWmrNfeEHSGSc3FyhNLlBU" />
     ...
     $OrdderAdres=array_unique($orderAdres);
     $WarreAdres=array_unique($wareAdres);
     echo 'OrderAdress' . '<br/>';
       vd($OrdderAdres);
      echo '<br/>'.'WareAdres' . '<br/>';
       vd($WarreAdres);
     //   $OrderAdrres= ArrayHelper::getColumn($orderAdres,'location.0', );
       // $WareAdrres= ArrayHelper::getColumn($wareAdres,'location.0', );
        $placeAdresses  = PlaceAdress::find()
            ->select(["id","location"])
            ->where([

                'id' =>[(int)$OrdderAdres[0]],
             //   'id' =>$this->_config['orderAddress'],

            ])
            ->all();
        
       $savedAdresses= ArrayHelper::getColumn($placeAdresses,'location.0', );
        var_dump($savedAdresses);
