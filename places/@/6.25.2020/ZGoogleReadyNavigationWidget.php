...
class ZGoogleReadyNavigationWidget1 extends ZWidget
{
    #region Congif
    /**
     * Configuration
    ...
    public $_layout = [
        'main' => <<<HTML
         <div class="row">
                ...
                <div class="col-md-12">
                            <i style="font-size:24px" class="fa" id="click_sidebar">&#xf0c9;</i>
                        <div id="right-panel" class="ctn"  style="float: left; margin-left: 1%; position: inherit; display: none; padding:4px;          
                width: 500px; height: 500px; overflow-x: hidden;overflow-y: auto;">
...
allMarkers = savedData.map(function(location, i) {
    ...
    var marker = new google.maps.Marker({
        position: {lat:parseFloat(location[0].lat), lng: parseFloat(location[0].lng)},
        label: 'Z' + i,
    });
    return marker;
});
...
function mapValueInstalization() {
    ...
    {
        $("<input hidden type='text' name = '{name}[" + index + "][place_id]' value='" + data[i][0].place_id + "'>").appendTo('#{id}-locations'); 
        $("<input hidden type='text' name = '{name}[" + index + "][address]' value='" + data[i][0].address+ "'>").appendTo('#{id}-locations');
        $("<input hidden type='text' name = '{name}[" + index + "][user_entered_address]' value='" + data[i][0].user_entered_address + "'>").appendTo('#{id}-locations'); 
        $("<input hidden type='text' name = '{name}[" + index + "][lat]' value=" + data[i][0].lat + ">").appendTo('#{id}-locations');
        $("<input hidden type='text' name = '{name}[" + index + "][lng]' value=" + data[i][0].lng + ">").appendTo('#{id}-locations');
        $("<hr>").appendTo('#{id}-locations');
        index++;
    }
    return data;
...
JS,
    ...
    public function codes()
    {
        ...
        $savedAdresses = ArrayHelper::getColumn($placeAdresses,'location');
...
&photoreference=CnRtAAAATLZNl354RwP_9UKbQ_5Psy40texXePv4oAlgP4qNEkdIrkyse7rPXYGd9D_Uj1rVsQdWT4oRz4QrYAJNpFX7rzqqMlZw2h2E2y5IKMUZ7ouD_SlcHxYq1yL4KbKUv3qtWgTK0A6QbGh87GB3sscrHRIQiG2RrmU_jF4tENr9wGS_YxoUSSDrYjWmrNfeEHSGSc3FyhNLlBU" />
        ...
        $depend_id .= $this->_config['depend']['depend_id'];

        if ($this->_config['isDepend'])
            $depend_id = str_replace($this->attributeAll, $this->_config['depend']['depend_id'], $this->id);
