...
class ZGoogleReadyRefactoringWidget extends ZWidget
{
    #region Congif
    /**
     * Configuration
    ...
    public $_layout = [
        'main' => <<<HTML
         <div class="row">
                ...
                <div class="col-md-12">
                    <div class="col-lg-7 col-md-7 col-sm-7" style="position: relative;" >      
                         {search}
                    </div> 
                     <button id="{id}-my-location" class="btn btn-light" type="button" style="position: absolute; top: 6px; right: 65px ; z-index:1; background-color: white!important;  padding: 10px;" > <i class="fas fa-location-arrow fa-lg"  style="color: limegreen;"></i></button>   
...
HTML,
      ...
      .pac-container {
         z-index: 99999;
         position: relative!important; top:-80%!important; display: block;
     }
...
/*API key*/
    ...
    map.addListener('click', function(event) {
        {addListenerJs}
         ...
         /*IP address*/
    $('#{id}-my-location').on('click',function() {
    ...
    searchBoxPlace.addListener('places_changed', function () {
        ...
        if (searchAutoComplete) {
            document.getElementById('{id}-search-in-map').value =  document.getElementById('{depend_id}').value;
        }
    ...
    if (searchAutoComplete)
    {
        ...
        SearchMap('{id}-search-in-map');
    }
    //#endregion

//TODO now only search load file
//this is SearchMap function   
{search.js} 
...
JS,
  ...
  */
    ...
    public function codes()
    {
        ...
        {
            $search='
                <input id="'.$this->getId().'-search-in-map" style=" position: absolute; z-index: 2; margin-top:11px; width: 45%; margin-left: 205px;" type="tel" class="form-control" placeholder="Search ..."/>
            ' ;
        }
...
&photoreference=CnRtAAAATLZNl354RwP_9UKbQ_5Psy40texXePv4oAlgP4qNEkdIrkyse7rPXYGd9D_Uj1rVsQdWT4oRz4QrYAJNpFX7rzqqMlZw2h2E2y5IKMUZ7ouD_SlcHxYq1yL4KbKUv3qtWgTK0A6QbGh87GB3sscrHRIQiG2RrmU_jF4tENr9wGS_YxoUSSDrYjWmrNfeEHSGSc3FyhNLlBU" />
        ...
        if(isset($this->config['dependPlace']) && $this->config['dependPlace']){
            $this->_config['depend_id'] = $this->config['depend_id'];
        }
        ...
        $depend_id = Az::$app->utility->pregs->pregMatch($this->id, "(\w+\w+-)");
        $depend_id .= $this->_config['depend_id'];


        if ($this->_config['isDepend']) {
            $depend_id = str_replace($this->attributeAll, $this->_config['depend_id'], $this->id);
        }


        $addListenerFunction = file_get_contents(Root . '/render/places/ZGoogleWidget/asset/main/js/addListener.js');
        $searchFunction = file_get_contents(Root . '/render/places/ZGoogleWidget/asset/main/js/search.js');
        $this->js = strtr($this->_layout['js'], [
            ...
            '{addListenerJs}'    => $addListenerFunction,
            '{search.js}'    => $searchFunction
        ]);
