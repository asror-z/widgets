...
class ZGoogleDbWidgetKH1 extends ZWidget
{
    #region Congif
    /**
     * Configuration
                        ...
                        <div class="specify-address  p-3 z-depth-1">
                            <div id="home" class="d-flex align-items-center">
                                ...
                                <div  class="text-icon d-flex flex-column">
                                    ...
                                    <p id="home_location"></p>  
                                </div>
                            </div>
                            <hr class="w-90">
                             <div id="work" class="d-flex align-items-center">
...
<!--                    </button>-->
                    <button class="map-button btn-car btn" id="DRIVING">
...
//Sidebar Start
                ...
                const mapAddressHidden = document.querySelector('.map-address-hidden');
                ...
                btnClose.addEventListener('click', () => {
                 ...
                 btnCar.addEventListener('click', () => {
                     mapAddressHidden.style.display = 'block';
                    ...
                    details.addEventListener('click', () => {
                     ...
                     // console.dir(mapAddressHidden.parentElement.lastChild.classList.add('abp-hidden'))
                     // console.log('hello')
                     // mapAddressHidden.parentElement.lastChild.classList.add('abp-hidden')
                     // mapAddressHidden.parentElement.lastChild.classList.toggle('abp-hidden')
                 }) ;
                     timesCircle.addEventListener('click', () => {
                     mapSaibar.style.display = 'block';
                     mapAddressHidden.style.display = 'none';
...
function createReadMap()
{
    ...
    trafficLayer.setMap(map);
     ...
     $('#home').click(function() {
        ...
        map.addListener('click', function(mouseEvent) {
            ...
            $('#home_location').css('margin-top','10px') ;
            $('#home_location').css('font-size','0.5rem') ;
            var loc = mouseEvent.latLng.toString() ;
            ...
            geocoder.geocode({'location': loc}, function(results,status) {
                ...
                $('.map-search-input').val(results[0].formatted_address) ;   
                        ...
                        $('#home_location').text('No results found');
                    }
                }
                else{
                }
      ...
      $('#work').on('click',function() {
         ...
         map.addListener('click', function(mouseEvent) {
            ...
            $('#work_adress').css('margin-top','10px') ;
            $('#work_adress').css('font-size','0.5rem') ;
            var loc = mouseEvent.latLng.toString() ;
            loc = loc.replace('(','').replace(')','').split(', ') ;
            loc = {lat: parseFloat(loc[0]), lng: parseFloat(loc[1])}
            console.log(loc[0]) ;
            geocoder.geocode({'location': loc}, function(results,status) {
                ...
                $('.map-search-input').val(results[0].formatted_address) ;   
                        ...
                        $('#work_adress').text('No results found');
                    }
                }
                else{
                }
    ...
    $('.move-closer , .button-share').click(function() {
        map.addListener('click', function(event) {
        ...
        //If second marker
         ...
         || (directionMarkerA && directionMarkerA.setMap && directionMarkerB && directionMarkerB.setMap && userWantChange === 'b')
            ...
            var loc2 = event.latLng.toString() ;
            loc2 = loc2.replace('(','').replace(')','').split(', ') ;
            loc2 = {lat: parseFloat(loc2[0]), lng: parseFloat(loc2[1])}
            console.log(loc2) ;
            
           
            geocoder.geocode({'location': loc2}, function(results,status) {
                if(status === 'OK'){
                    if (results[0]) {
                        map.setZoom(13);
                      
                    loc2 = results[0].formatted_address ;
                    
                $('#direction_b').val(results[0].formatted_address);
                 
                    } 
                    else {
                        $('#direction_b').val('No results found');
                    }
                }
                else{
                }
        ...
        //If first marker
            ...
            userWantChange = 'b';
            var loc1 = event.latLng.toString() ;
            loc1 = loc1.replace('(','').replace(')','').split(', ') ;
            loc1 = {lat: parseFloat(loc1[0]), lng: parseFloat(loc1[1])}
            console.log(loc1) ;
            
           
            geocoder.geocode({'location': loc1}, function(results,status) {
                if(status === 'OK'){
                    if (results[0]) {
                        map.setZoom(13);
                        
                    loc1 = results[0].formatted_address ;
                $('#direction_a').val(results[0].formatted_address);
                 
                    } 
                    else {
                        $('#direction_a').val('No results found');
                    }
                }
                else{
                }
        ...
        /*$('.fa-sync').on('click',function() {
            ...
            if (count===0){
                ...
                var s1 = document.getElementById("direction_a").value;
                var s2 = document.getElementById("direction_b").value;
               console.log(s1 + " clicking") ;
               console.log(s2 + " clicking") ;
               document.getElementById("direction_b").value = s1;
               document.getElementById("direction_a").value = s2;
               $('#direction_b').trigger('change');
               $('#direction_a').trigger('change');
               $('#direction_b').val(s1) ;
               $('#direction_a').val(s2) ;
               ...
               if (directionMarkerA && directionMarkerA.setMap && directionMarkerB && directionMarkerB.setMap)
                {
                    console.log('drawing if')
                    var LatLng1 = directionMarkerA.getPosition();
                    var LatLng2 = directionMarkerB.getPosition();
            
                    directionMarkerA.setMap(null); 
                    directionMarkerB.setMap(null); 
                    createDirection(LatLng2, LatLng1);
                    console.log('drawn if')
                    alert("Alert")
                }
                count = 1;
            }
            else{
                const temp =  $('#direction_a').val() ;
                
               $('#direction_a').val(loc1) ;
               var test = 123 ;
               $('#direction_b').val(loc2) ;
               console.log(loc1 + " come on else") ;
...
$('.fa-sync').on('click',function() {
    ...
    if (directionMarkerA && directionMarkerA.setMap && directionMarkerB && directionMarkerB.setMap)
    {
        ...
        var changerDirection = directionMarkerA;
        directionMarkerA = directionMarkerB;
        directionMarkerB = changerDirection;
        createDirection(LatLng2, LatLng1);
        console.log('drawn if')
    }
