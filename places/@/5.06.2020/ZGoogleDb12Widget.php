...
class ZGoogleDb12Widget extends ZWidget
{
    #region Congif
    /**
     * Configuration
    ...
    bootbox.prompt({
        ...
        callback: function(result){ 
            ...
            .then(data => {
                ...
                if (type === 'push') {
                    createHiddenInput(index,'lat',lat);
                    createHiddenInput(index,'lng',lng);
                    createHiddenInput(index,'place_id',place_id);
                    createHiddenInput(index,'address',address);
                    createHiddenInput(index,'user_entered_address',result);
...
/*For create new hiddem input*/
function createHiddenInput(i_thIndex , name, value)
{
    $("<input type='text' name = '{name}["+ i_thIndex + "]["+ name + "]' value=" + value + ">").appendTo('#locations-{id}');
} 
...
window.onload = function() {
    ...
    for(var i = 0; i < savedData.length; i++)
    {
        ...
        createHiddenInput(cnt_ind,'lat',lat);
        createHiddenInput(cnt_ind,'lng',lng);
        createHiddenInput(cnt_ind,'place_id',place_id);
        createHiddenInput(cnt_ind,'address',address);
        createHiddenInput(cnt_ind,'user_entered_address',result);
        cnt_ind++;
    }
    createMap();
}   
