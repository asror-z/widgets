...
class ZGoogleReadyNavigationWidgetZafar extends ZWidget
{
    #region Congif
    /**
     * Configuration
        ...
        'main' => <<<HTML
             <!--                '{depend}'-->
                <div id="{id}-locations">
                    ...
                    <!--search header-->
                    <div id="{id}-right-panel"class="ctn map-sidebar-back w-100" ><div class="map-sidebar hidden-map-sidebar"> 
                   <div class="map-search-in d-flex justify-content-around align-items-center pt-3">
                    ...
                    <div id="{id}-directions" class="map-search-from"></div>
                      </div>                      
                </div>
                  ...
                  <i id="{id}-Openbtn" class="openbtn d-flex align-items-center justify-content-center"><i class="fas fa-chevron-right"></i>
                  </i> 
    ...
    callback: function (result) {
        ...
        if (event === true) {
                ...
                function createReadMap()
                {
                    ...
                    setInterval(optimalPoints(), 10000);
                    //#endregion
                }
                //#endregion
                ...
                $('#{id}-click_sidebar').on('click', function() {
          ...
          navigator.geolocation.getCurrentPosition(function(position) {
            ...
            .then(data => {
                ...
                console.log(place_id );
                 ...
                 $("<input  type='text' name = '{name}[" + index + "][place_id]' value='" + place_id + "'>").appendTo('#{id}-locations'); 
                    $("<input  type='text' name = '{name}[address]' value='" + address + "'>").appendTo('#{id}-locations');
                    $("<input  type='text' name = '{name}["+ index + "][lat]' value=" + position.coords.latitude+ ">").appendTo('#{id}-locations');
                    $("<input  type='text' name = '{name}["+ index + "][lng]' value=" + position.coords.longitude + ">").appendTo('#{id}-locations');
                    $("<hr>").appendTo('#{id}-locations');               
                console.log('address '); 
...
JS,
    ...
    public function asset()
    {
        ...
        $this->fileCss('\render\places\ZGoogleWidget\asset\main\css\styleZ.css');
    }
