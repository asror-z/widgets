 ...
 */
class ZMultiWidget extends ZWidget
{
    ...
    public $_event = [
        ...
        'afterDeleteRow' => 'console.log("asfsadfasdfsafd");',
    ...
    public const renderer = [
        ...
        'tablenative' => TableRenderer::class,
  ...
  $('.js-input-remove').on('click', function() {
        ...
        window.index = parentOne.parent('tr').data('index');
        console.log('index' + window.index);
        
          var url = '/api/core/save/remove.aspx';
               ...
               var xhr = $.ajax({
                    ...
                    success: function (data, status, xhr) {
                        ...
                        console.log('Remove success! Index: ' + index);
                        
                       if ($.sendPjax !== undefined)
                        $.sendPjax();
                    },
...
JS
        ...
        //  vd($model->columns['shop_option_type_id']->data);
        ...
        if (empty($this->columns))
            foreach ($model->columns as $key => $column) {
                ...
                $item['name'] = $key;
                $item['type'] = ZHInputWidget::class;
                $item['title'] = $column->title;

                $item['options']['model'] = $model;
                $item['options']['attribute'] = $key;
...
CSS;
        ...
        $this->js = strtr($this->_layout['main'], [
            '{parentClass}' => $model->allApp()->parentClassName,
            '{tranz}' => $this->paramGet(paramTransact),
            '{parentAttr}' => $model->allApp()->parentAttr,
            '{parentId}' => $model->allApp()->parentId,
        ]);
