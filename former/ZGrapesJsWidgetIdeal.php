...
class ZGrapesJsWidgetIdeal extends ZWidget
{
    ...
    public $_config = [
        ...
        'tooltipTheme' => ZGrapesJsWidgetBosya::tooltipTheme['tooltipster-shadow'],
    ];
   ...
   // editor.CssComposer.getAll().reset();
    ...
    panelManager.addButton('views', {
        ...
        command: function() {
            ...
            const contain = $('.gjs-pn-views-container');
            const bodyCanvas = $('.gjs-cv-canvas');
            const hideClass = $('.hiding');
            
            contain.toggleClass('hide');
            bodyCanvas.toggleClass('w-100');
    
            if (contain.hasClass('hide')) {
                setTimeout(function() {
                    hideClass.toggleClass('gjs-pn-active gjs-four-color');
                    hideClass.removeClass('fa-arrow-right');
                    hideClass.addClass('fa-arrow-left');
                }, 100);
            }else {
        ...
        tagName:'span', 
        attributes: {
            class: 'gjs-pn-button hiding grape-tooltip',
            title: 'Close Menu'
        },
    });
    ...
    editor.on('load', function(model) {
     ...
     var myCss = `{css}`;
     var replaceCss = myCss.replace(/\.|{.*}/gs, '')
...
JS,
    ...
    public function asset()
    {
        ...
        $this->fileCss('/render/former/ZGrapesJsWidget/demo/Grapes/css/clean.css');
        #endregion
        #region Js
    ...
    public function codes()
    {
        ...
        $mainCode = file_get_contents($root . 'mainJs.js');
        $events = file_get_contents($root . 'events.js');
    ...
    public function getTopButtons($href)
    {
        ...
        $btn->attributes = [
            'title' => Az::l('Показать рамки'),
            'class' => 'WhiteClass'
        ];
        $btn->tagName = 'span';
        $btn->icon = 'fas fa-square';
        $btn->label = '';
        $btn->active = true;
        $btn->command = 'sw-visibility';
        $topButtons['sw-visibility'] = $btn;*/
        ...
        if (!empty($this->_config['buttons'])) {
            $topButtons = $this->_config['buttons'];
        }
