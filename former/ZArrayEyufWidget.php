...
use zetsoft\service\forms\DyGrid;
 ...
 */
class ZArrayEyufWidget extends ZDynaEyufWidget
{
    ...
    public function codes()
    {
        ...
        foreach ($this->model->columns() as $key => $column) {
            ...
            if ($this->_config['pageSummaryTotal'])
                ...
                $summary = (new DyGrid())->summaryCheck($key, $column, $configs);
            ...
            if (!empty($configs->roleDeny))
                foreach ($configs->roleDeny as $deny) {
                    if ($this->hasRole($deny))
                        continue;
                }
        ...
        $this->options = ZArrayHelper::merge($this->options, [
            ...
            'gridOptions' => [
                ...
                'showPageSummary' => $this->model->configs->summary,
                'panel' => [
